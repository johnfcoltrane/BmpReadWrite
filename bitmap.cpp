...
//struct Image_st::Image_st(Image_st& image)
Image_st::Image_st()
{

}
Image_st::Image_st(Image_st& img)
{
	height = img.height;
	width  = img.width;
	size   = img.size;
	data   = (Rgb*) malloc(size);
	size_t n = size / sizeof(Rgb);
	for(int i=0; i<n; i++){
		data[i] = img.data[i];
	}
}
...
Image *Read_Bmp(char *filename)
{
	...
	for(i=0; i<height; i++){
		...
		for(j=0; j<width; j++){
			img->data[(height-i-1)*width + j].b = bmp_line_data[j*3 + 0];
...
Image *Create_Image(int width, int height)
{
	...
	//
	// char* img->data(width * height) (data部分全体)をnew
	//
	img->size = (int) (sizeof (Rgb) * width * height);
